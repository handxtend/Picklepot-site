...
function renderRegistrations(entries){
  ...
  const typeHtml = isSiteAdmin() ? (
    ...
    const buyinRaw = (typeof e.applied_buyin==='number') ? e.applied_buyin : Number(e.applied_buyin||0);
  const buyin = dollars(buyinRaw);
...
/* =========================================================
   LOCKED FEATURE: Type↔Buy‑in toggle (do not remove)
   Ensures the registration Type select always updates Buy‑in
   and persists to Firestore, regardless of future edits.
   ========================================================= */
(function ToggleTypeLock(){
  'use strict';
  try{
    if (window.__TYPE_TOGGLE_LOCKED) return;
    Object.defineProperty(window, '__TYPE_TOGGLE_LOCKED', {
      value: true, enumerable: false, configurable: false, writable: false
    });
  }catch(_){ return; }

  var $dollars = (window.dollars || (function(n){ return '$' + Number(n||0).toFixed(2); }));

  async function getPot(){
    try{
      var p = (window.CURRENT_DETAIL_POT || null);
      if (p && p.id) return p;
      var id = (document.getElementById('v-pot') && document.getElementById('v-pot').value) ||
               (document.getElementById('j-pot-select') && document.getElementById('j-pot-select').value) || '';
      if (!id || !window.db) return null;
      var snap = await db.collection('pots').doc(id).get();
      if (snap && snap.exists){
        p = { id: snap.id, ...(snap.data()||{}) };
        try{ window.CURRENT_DETAIL_POT = window.CURRENT_DETAIL_POT || p; }catch(_){}
        return p;
      }
    }catch(e){ console.warn('[TypeLock] pot fetch failed', e); }
    return null;
  }

  async function onTypeChange(ev){
    var t = ev.target;
    if (!t || !(t.matches && (t.matches('select.mtSel') || t.matches('select[data-act="type"]')))) return;
    ev.stopPropagation();

    var entryId = t.getAttribute('data-id');
    if (!entryId) return;

    var pot = await getPot();
    if (!pot || !window.db) { console.warn('[TypeLock] no pot/db'); return; }

    var isMember = (/^m/i.test(t.value)) || (t.value === 'Member');
    var buyin = isMember ? Number(pot.buyin_member||0) : Number(pot.buyin_guest||0);

    // Update the UI immediately
    try{
      var row  = t.closest('tr');
      var cell = row ? row.querySelector('.buyin') : null;
      if (cell) { cell.textContent = $dollars(buyin); cell.setAttribute('data-value', String(buyin)); }
    }catch(_){}

    // Persist (primary: by entry id)
    try{
      await db.collection('pots').doc(pot.id).collection('entries').doc(entryId)
        .update({ member_type: isMember?'Member':'Guest', applied_buyin: buyin });
      return;
    }catch(errPrimary){
      console.warn('[TypeLock] primary update failed, attempting email fallback', errPrimary);
    }

    // Fallback: match by email in this row
    try{
      var emailCell = t.closest('tr')?.querySelector('td:nth-child(2)');
      var emailText = (emailCell && emailCell.textContent || '').trim();
      if (emailText){
        var q = await db.collection('pots').doc(pot.id).collection('entries')
          .where('email','==', emailText).limit(1).get();
        if (!q.empty){
          var altId = q.docs[0].id;
          await db.collection('pots').doc(pot.id).collection('entries').doc(altId)
            .update({ member_type: isMember?'Member':'Guest', applied_buyin: buyin });
        } else {
          console.warn('[TypeLock] email fallback found no match for', emailText);
        }
      }
    }catch(errFb){ console.warn('[TypeLock] email fallback failed', errFb); }
  }

  function bind(){
    try{
      var tbody = document.querySelector('#adminTable tbody');
      if (!tbody || tbody.__typeToggleLockBound) return;
      // Capture phase to intercept before other listeners
      tbody.addEventListener('change', onTypeChange, true);
      Object.defineProperty(tbody, '__typeToggleLockBound', {
        value: true, enumerable: false, configurable: false, writable: false
      });
    }catch(e){ console.warn('[TypeLock] bind failed', e); }
  }
