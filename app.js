...
(function(){
  ...
  // Reuse helpers from roster module if present
  const hasRosterHelpers = (typeof window.loadRoster === 'function') && (typeof window.isOnRoster === 'function' || true);
  function computeAutoType(){
    const p = window.CURRENT_JOIN_POT;
    if (!p) return null;
    const fname = $('#j-fname')?.value || '';
    const lname = $('#j-lname')?.value || '';
    const email = $('#j-email')?.value || '';
    const full  = clean(fname + ' ' + lname);
    // Try isOnRoster if available, else fallback to simple name/email lookup via loadRoster
    try{
      ...
      if (typeof window.loadRoster === 'function'){
        const r = window.loadRoster(p.id);
        const n = (full||'').toLowerCase();
        const e = (email||'').toLowerCase();
        const on = (r && (r.names||[]).includes(n)) || (r && (r.emails||[]).includes(e));
  ...
  function forceTypeInUI(autoType){
    if (!autoType) return;
    const sel = $('#j-mtype');
    if (sel){ sel.value = autoType; safe(()=>sel.dispatchEvent(new Event('change',{bubbles:true})) ); }
  ...
  window.startJoinCheckout = async function(){
    // Enforce roster rule just-in-time for Stripe
    const autoType = computeAutoType();
    forceTypeInUI(autoType);
    // If your code uses a hidden amount field, recompute it after forcing the type
    try{
      if (typeof window.computeJoinAmount === 'function'){ window.computeJoinAmount(); }
    }catch(_){}
    return await __orig_startJoinCheckout.apply(this, arguments);
  };

  // Also recompute when Payment Type is switched to Stripe
  (function bindPayChange(){
    const pay = $('#j-paytype');
    if (!pay || pay.__autoBound) return;
    pay.__autoBound = true;
    pay.addEventListener('change', function(){
      if ((this.value||'').toLowerCase()==='stripe'){
        const t = computeAutoType(); forceTypeInUI(t);
      } else {
        safe(()=>window.updateJoinCost && window.updateJoinCost());
      }
    });
