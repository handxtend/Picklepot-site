...
// == Admin Owner Tool: robust delegated handlers & rotate ==
(function(){
  function _getDb(){
    try{
  ...
  async function _rotateOwnerCode(){
    try{
      ...
      var code = (typeof randomOwnerCode==='function') ? randomOwnerCode(8) : Math.random().toString(36).slice(-8).toUpperCase();
      var salt = (typeof randomOwnerCode==='function') ? randomOwnerCode(12) : Math.random().toString(36).slice(-12).toUpperCase();
      var hasher = (typeof sha256Hex==='function') ? sha256Hex : async (s)=>{
        const enc = new TextEncoder().encode(s);
        const buf = await crypto.subtle.digest('SHA-256', enc);
        return Array.from(new Uint8Array(buf)).map(b=>b.toString(16).padStart(2,'0')).join('');
      };
      var hash = await hasher(code + ':' + salt);
      ...
      var mk = (typeof makeManageLink==='function') ? makeManageLink : (function(p,c){
        const base = location.origin.replace(/\/$/,'');
        return base + '/manage.html?pot=' + encodeURIComponent(p) + '&oc=' + encodeURIComponent(c);
      });
      var link = mk(pid, code);
      ...
      console.error('Rotate owner code failed', e);
      alert('Rotate failed. Check console.');
    }
  }
  ...
  document.addEventListener('click', function(ev){
    ...
    if(!target) return;
    var rot = target.closest && target.closest('#btn-owner-rotate');
    if(rot){ ev.preventDefault(); _rotateOwnerCode(); return; }
    var show = target.closest && target.closest('#btn-owner-link');
    if(show){
      ev.preventDefault();
      var out = document.getElementById('owner-output');
      if(out){
