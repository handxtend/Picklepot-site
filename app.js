...
// === Roster CSV dynamic loader (Admin + Organizer Edit) ===
(function(){
  ...
  async function loadRoster(file, statusEls){
    try{
      const txt = await file.text();
      const names = parseCSV(txt);
      window.__ROSTER_SET = new Set(names);
      const msg = names.length ? ('Roster loaded: ' + names.length + ' names') : 'No roster loaded';
      (statusEls||[]).forEach(el=>{ if(el) el.textContent = msg; });
      if (typeof window.applyRosterEligibility==='function') window.applyRosterEligibility();
    }catch(e){
      console.error('CSV load failed', e);
      (statusEls||[]).forEach(el=>{ if(el) el.textContent = 'Failed to load CSV'; });
    }
  }
  function collectStatusEls(input){
    const map = {
    ...
    const id = map[input.id];
    const el = id ? document.getElementById(id) : null;
    return el ? [el] : [];
  }
  function bindInput(el){
    if(!el || el.__rosterBound) return;
    el.addEventListener('change', (ev)=>{
      const f = ev.target.files && ev.target.files[0];
      if(f) loadRoster(f, collectStatusEls(el));
    });
    el.__rosterBound = true;
  }
  function scan(){
    document.querySelectorAll('#admin-roster-csv,#org-roster-csv,#c-roster-csv,#m-roster-csv,#e-roster-csv').forEach(bindInput);
  }
  try{ scan(); }catch(_){}
