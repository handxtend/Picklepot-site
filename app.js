...
// --- Organizer manage-link auth (no layout changes) ---
window.OWNER_OK = window.OWNER_OK || false;
async function verifyOwner(potId){
  try{
    const params = new URLSearchParams(location.search);
    const key = params.get('key') || localStorage.getItem('owner_key_'+potId);
    if(!key) return false;
    const r = await fetch((window.API_BASE||'') + '/pots/' + encodeURIComponent(potId) + '/owner/auth', {
      method:'POST',
    ...
    try{ if (typeof refreshAdminUI==='function') refreshAdminUI(); }catch(_){}
    return true;
...
function attachActivePotsListener(){
  ...
  try{
    JOIN_POTS_SUB = db.collection('pots').where('status','in',['open','active']).onSnapshot(snap=>{
      const now = Date.now();
      const pots = [];
      snap.forEach(d=>{
...
async function onLoadPotClicked(){
  ...
  try{ verifyOwner(pot.id); }catch(_){}
