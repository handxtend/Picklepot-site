...
function onJoinPotChange(){
  ...
  try{ if(window.applyRosterEligibility) window.applyRosterEligibility(); }catch(_){}
  ...
  try{
    ...
    if (__effective_pay_type === 'Stripe'){
      ...
      const payload = {
        ...
        cancel_url: origin + '/cancel.html?flow=join&session_id={CHECKOUT_SESSION_ID}&pot_id=' + p.id + ',
        method: 'stripe'
      };
...
// --- Roster-based defaulting on the Join page ---
(function(){
  if (typeof window.applyRosterEligibility !== 'function') {
    window.applyRosterEligibility = function applyRosterEligibility(){
      try{
        const p = (typeof CURRENT_JOIN_POT!=='undefined') ? CURRENT_JOIN_POT : null;
        const sel = document.getElementById('j-mtype');
        if(!p || !sel) return;
        const hasRoster = Array.isArray(p.roster_names) && p.roster_names.length>0;
        ...
        if (isMember){
          sel.value = 'Member';
          Array.from(sel.options).forEach(o=>{ if(o.value==='Guest') o.disabled = true; if(o.value==='Member') o.disabled = false; });
        }else{
          sel.value = 'Guest';
          Array.from(sel.options).forEach(o=>{ if(o.value==='Member') o.disabled = true; if(o.value==='Guest') o.disabled = false; });
        }
        try{ if (typeof updateJoinCost==='function') updateJoinCost(); }catch(_){}
      }catch(e){ console.warn('applyRosterEligibility error', e); }
    };
  }
  function bindNameInputs(){
    var a = document.getElementById('j-fname');
    var b = document.getElementById('j-lname');
    if(a && !a.__rosterBind){ a.addEventListener('input', window.applyRosterEligibility); a.__rosterBind=true; }
    if(b && !b.__rosterBind){ b.addEventListener('input', window.applyRosterEligibility); b.__rosterBind=true; }
  }
  document.addEventListener('DOMContentLoaded', bindNameInputs);
  try{ bindNameInputs(); }catch(_){}
  new MutationObserver(bindNameInputs).observe(document.documentElement||document.body, {childList:true, subtree:true});
})();
