...
// --- Robust CSV roster hook (works on create & manage pages) ---
(function(){
  ...
  function parseCSV(text){
    ...
    for (let i=start;i<lines.length;i++){
      ...
      const key = normalizeName(cols[fi]||'', cols[li]||'');
  ...
  function bindCsvOnce(){
    const input = document.getElementById('c-roster-csv');
    if (input && !input.__csvBound){
      input.addEventListener('change', (e)=>{
        const f = e.target.files && e.target.files[0];
        if (f) handleCSVFile(f);
      });
      input.__csvBound = true;
    }
  }
  try{ bindCsvOnce(); }catch(_){}
  document.addEventListener('DOMContentLoaded', bindCsvOnce, { once: true });
})();
