...
// === Roster CSV loader (works on Create/Manage/Edit) ===
(function(){
  ...
  function parseCSV(text){
    ...
    let li = header.findIndex(h=>/^last/.test(h));
    let start = 1;
    if (fi === -1 || li === -1){ fi = 0; li = 1; start = 0; } // No header
    const names = [];
  ...
  async function loadFile(file, statusEl){
    try{
      const text = await file.text();
      const names = parseCSV(text);
      window.__ROSTER_SET = new Set(names);
      ...
      console.error('Roster CSV parse failed', err);
      if (statusEl) statusEl.textContent = 'Failed to load CSV';
    }
  }
  function getStatusForInput(input){
    ...
    const id = map[input.id] || null;
    return id ? document.getElementById(id) : null;
  }
  function bindRosterInputs(){
    const inputs = Array.from(document.querySelectorAll('#c-roster-csv,#m-roster-csv,#e-roster-csv'));
    inputs.forEach(inp=>{
      if (inp && !inp.__rosterBound){
        inp.addEventListener('change', (e)=>{
          const f = e.target.files && e.target.files[0];
          const statusEl = getStatusForInput(inp);
          if (f) loadFile(f, statusEl);
        });
        inp.__rosterBound = true;
      }
    });
  }
  try{ bindRosterInputs(); }catch(_){}
  document.addEventListener('DOMContentLoaded', bindRosterInputs, {once:true});
})();
